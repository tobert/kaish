...
- **`awk exit N` sets the builtin's exit code** (`awk 'BEGIN{exit 3}'` → exit 3); the code was previously dropped and awk always exited 0. Prior output and `END` blocks still run, and `exit` inside `END` overrides the code (matching awk).
- **`awk -v NAME=VALUE` (and `-v a=1 -v b=2`) now work in the natural unquoted form**, and every `-v` is applied (not just the last). `NAME=VALUE` lexes as an assignment token, which a value-taking flag previously skipped — so `-v` grabbed the *program* instead and the assignment leaked out as a stray argument (it only worked when the value was quoted). A value flag now consumes a following `key=value` token as its argument (getopt semantics), which also fixes the same class for other builtins' value flags.
