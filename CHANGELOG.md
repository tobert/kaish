...
### Fixed
- **`awk -F:` (and `-F;`, `-F|`, `--field-separator=:`) no longer errors at the parse layer**: the lexer's post-processing now fuses a span-adjacent shell metacharacter directly onto a short flag into a single `ShortFlag` token (e.g. `ShortFlag("F:")`), so the canonical `awk -F: '{print $1}' /etc/passwd` invocation parses and executes correctly. The fix is guarded by span-adjacency — a space-separated `cmd -F :` still tokenizes as two tokens so `;` as a statement terminator and `|` as a pipe are never affected. Additionally, bare `:` in argument position (e.g. `--field-separator=:`) is now a valid literal expression, mirroring the existing `,` handling.
