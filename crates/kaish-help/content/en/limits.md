...
- **`sort < f > f` is refused** (exit 1) instead of emptying `f` as bash does. Write to a temp file, then `mv` it over `f`.
...
| Bypass VFS sandbox | Set `allow_unwrapped_commands=false` to block PATH lookup, `exec`, `spawn`, and `env CMD` |
