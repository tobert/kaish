...
//! `allow_unwrapped_commands` chooses between "no word spawns anything" and
//! "every program on `$PATH` spawns, with any arguments" — it allows any
//! program that is not a wrapped command: PATH lookup, `exec`, and `spawn`.
//! A wrapped command is the setting between: the executable is pinned at
//! registration, so a changed `$PATH` changes nothing, and an undeclared verb
//! or flag is refused before any spawn. Registering wrappers with the switch
//! left `false` is the intended shape, not a way around it.
//!
//! ```no_run
