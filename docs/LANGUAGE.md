...
`export`, a `for` loop head, `env`, `read`, `unset`, `push`, and
`scatter --as`.
...
```sh
# seq returns text — the newline split iterates over the numbers
for i in $(seq 1 5); do
    echo "Number: $i"
done
...
- **`sed` is a "muscle-memory" subset, not full sed** — kaish's `sed` deliberately implements the slice of GNU/BSD (AT&T) `sed` that humans and agents actually reach for by reflex — closest in spirit to **busybox** `sed`, which is a strong influence (the supported set was [chosen from a cross-model usability panel](designing-syntax-with-llms.md)). It covers:
  ...
  - **Regex is GNU BRE by default, exactly as GNU sed does** — same dialect as `grep` (see the `grep` entry above): bare `( ) { } | + ?` are literal, `\(…\)` (groups), `\|` (alternation), `\{N,M\}` (intervals), and `\+`/`\?` (quantifiers) are operators, so `s/fn consult(/x/` matches the literal text and agent-idiomatic `s/cat\|dog/X/` / `s/\(a\)\(b\)/\2\1/` alternate/capture. `\n \t \r \a \f \v` are real control characters, in both dialects (unlike `grep`, which reads them as a stray backslash + the literal letter). Pass **`-E`/`-r`** for strict ERE, where the bare forms are the operators and those escapes are literal — the escape hatch for a literal `|`/`+` — and the same GNU escape table applies there too: `s/\d/X/` under `-E` still matches the literal `d`, exactly as `grep -E`'s does, unlike the regex engine's own Perl-style digit class. A pattern-side backreference (`\1` in the pattern) is still rejected in both dialects — the linear-time engine has none, though real GNU sed supports one under `-E` when a group precedes it. `awk` does **not** share this dialect: it has no BRE at all and reads gawk's ERE instead (see the `awk` entry below).
...
- **`awk` reads gawk's ERE exactly, exactly as gawk does** — awk has no BRE at all, unlike `grep`/`sed`, so this is the *opposite* reading of the same escapes: bare `( ) { } | + ?` are operators (`/(GET|POST)/`, `/a{2,5}/`), and `\( \) \{N,M\} \| \+ \?` match the literal character (`/fn consult\(/`, `gsub(/a\|b/, "X")` on the literal text `a|b`). A bare `{` is literal unless a digit follows it immediately (`/a{x}/` is literal, `/a{2}/` is the interval). `\<` `\>` `` \` `` `\'` `\y` are GNU word/buffer-boundary escapes; `\b` is a literal backspace, not a word boundary (`\y` is). `\d`/`\D` are not gawk regexp operators — the literal letter. `-F` and `-v` run through the same string-escape rules as a program string literal before the value is used, so `-F '\|'` is a literal `|`. **No back-references** (`\1`-`\9`) — the regex engine has none, and real gawk reads those as octal escapes rather than back-references anyway. **No operand `var=value` assignment** — real gawk reads `awk '{print x}' x=1 file` as an assignment that runs when gawk reaches that operand (so it can sit between file operands, setting a variable partway through); kaish reads `x=1` as a file path instead, a documented gap, not a silent stand-in for gawk's behavior.
